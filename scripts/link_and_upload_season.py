...
def get_team_abbreviation(team_id, season=None):
    """Get the canonical team abbreviation from the teams table, with fallback to team_seasons."""
    ...
    try:
        ...
        print(f"⚠️ Error getting team abbreviation from teams table for {team_id}: {str(e)}")
    
    # Fallback: try to get abbreviation from team_seasons for the current season
    if season:
        try:
            response = team_seasons_table.query(
                IndexName='team_id-season-index',
                KeyConditionExpression=Key('team_id').eq(team_id) & Key('season').eq(int(season))
            )
            if response.get('Items'):
                # Get the most recent season entry (should only be one for this season)
                abbr = response['Items'][0].get('abbreviation')
                if abbr:
                    print(f"ℹ️ Found abbreviation '{abbr}' for {team_id} from team_seasons (season {season})")
                    return abbr
        except Exception as e:
            print(f"⚠️ Error querying team_seasons for {team_id} (season {season}): {str(e)}")
    
    print(f"❌ Error getting team abbreviation for {team_id}: Team exists but has no abbreviation set")
    return None

# ---- Utility Functions ----
def normalize_result(result):
    """Normalize match result to a standard format."""
    if not result:
        return "UNKNOWN"
    result = result.upper().strip()

    # Simplified result normalization - just standardize the format
...
def generate_match_id(winner_id, loser_id, date, result, season):
    """Generate a match ID using the format: season-winner_id-loser_id-result-date."""
    # Use the season parameter directly
    return f"{season}-{winner_id}-{loser_id}-{result}-{date}"
...
def process_folder(folder_path, is_base_year=False):
    ...
    for i, file_path in enumerate(files, 1):
        ...
        for wrestler in team_data['roster']:
            # Get or create season_wrestler_id
            if 'season_wrestler_id' in wrestler:
                season_wrestler_id = str(wrestler['season_wrestler_id'])
            else:
                print(f"\n⚠️ Warning: No season_wrestler_id provided for {wrestler['name']}")
                team_abbr = get_team_abbreviation(team_id, season)
                if not team_abbr:
                    print(f"\n❌ CRITICAL ERROR: No abbreviation found for team {team_id}")
                    team_abbr = team_id
                season_wrestler_id = f"{season}-{team_abbr}-{wrestler['name'].replace(' ', '_')}-{wrestler.get('weight_class', 'UNKNOWN')}"
            ...
            if not is_base_year:
                ...
                print("\n" + "="*80)
                print(f"🔍 EVALUATING WRESTLER: {wrestler['name']}")
                print("="*80)
                print(f"Current Details:")
                print(f"  Name: {current['name']}")
                print(f"  Team: {current['team_name']} ({current['team_id']})")
                print(f"  Class Year: {current['class_year']}")
                print(f"  Weight: {current['weight_class']}")
                print(f"\nMatching Criteria:")
                
                # Calculate and show all matching criteria
                is_freshman = current['class_year'] in {'FR', 'RSFR', 'Fr.', 'RS Fr.'}
                print(f"  Is Freshman: {is_freshman}")
                
                if match:
                    print(f"\nBest Match Found:")
                    print(f"  Name: {match['name']}")
                    print(f"  Team: {match.get('team_name', '')} ({match.get('team_id', '')})")
                    print(f"  Class Year: {match.get('class_year', '')}")
                    print(f"  Weight: {match.get('weight_class', '')}")
                    
                    # Calculate detailed matching scores
                    name_sim = SequenceMatcher(None, current['name'].lower(), match['name'].lower()).ratio()
                    exact_name = normalize(current['name']) == normalize(match['name'])
                    team_match = current['team_id'] == match.get('team_id')
                    
                    try:
                        weight_diff = abs(int(current['weight_class']) - int(match.get('weight_class', 0)))
                        weight_match = "Full" if weight_diff <= 10 else "Partial" if weight_diff <= 20 else "None"
                    except:
                        weight_diff = 999
                        weight_match = "Unknown"
                    
                    print(f"\nMatch Details:")
                    print(f"  Name Similarity: {name_sim:.2f}")
                    print(f"  Exact Name Match: {exact_name}")
                    print(f"  Team Match: {team_match}")
                    print(f"  Weight Difference: {weight_diff}lbs ({weight_match})")
                    ...
                    print("\nNo match found in previous season")
                    action, confidence_score = "AUTO New Career", 0
                    print(f"\nEvaluation Decision:")
                    print(f"  Action: {action}")
                    print(f"  Confidence Score: {confidence_score:.1f}")
                
                # Removed pause for review
            else:
            ...
            for match in wrestler.get('matches', []):
                ...
                # Get current wrestler's ID (should always be present)
                if not wrestler.get('season_wrestler_id'):
                    raise Exception(f"❌ CRITICAL: Current wrestler missing season_wrestler_id:\n{json.dumps(wrestler, indent=2)}")
                
                current_wrestler_id = str(wrestler['season_wrestler_id'])
                opponent_id = match.get('opponent_id')
                
                # Special cases for opponent_id
                if not opponent_id or opponent_id == "-1":
                    ...
                    if opponent_name == "BYE":
                        opponent_id = "BYE"
                    elif opponent_name == "Unknown" or not opponent_name:
                        opponent_id = "UNKNOWN"
                    elif not opponent_team or opponent_team.upper() in ["UNATTACHED", "UNAT"]:
                        # Format: year-UNAT-name for unattached wrestlers
                        opponent_id = f"{season}-UNAT-{opponent_name.replace(' ', '_')}"
                    else:
                        # Get team info for the opponent
                        ...
                        opponent_team_abbr = get_team_abbreviation(opponent_team_id, season)
                        if not opponent_team_abbr:
                            raise Exception(f"❌ CRITICAL: Missing team abbreviation for opponent {opponent_team_id}")
                        
                        # Format: year-team_abbr-name for team wrestlers
                        opponent_id = f"{season}-{opponent_team_abbr}-{opponent_name.replace(' ', '_')}"
                        print(f"ℹ️ Created fallback ID for opponent: {opponent_id}")

                # Assign winner and loser IDs based on who won
                if is_wrestler1_winner:
                    winner_id = current_wrestler_id
                    loser_id = opponent_id
                else:
                    winner_id = opponent_id
                    loser_id = current_wrestler_id

                # Debug logging
                print(f"\n🔍 Match ID Debug:")
                print(f"  Current Wrestler: {wrestler['name']} (ID: {current_wrestler_id})")
                print(f"  Is Winner: {is_wrestler1_winner}")
                print(f"  Winner Name: {match.get('winner_name')}")
                print(f"  Loser Name: {match.get('loser_name')}")
                print(f"  Opponent ID: {opponent_id}")
                print(f"  → Winner ID: {winner_id}")
                print(f"  → Loser ID: {loser_id}")

                # Get match details and create match ID
                date = match.get('date', 'unknown')
                result = match.get('result', '')
                match_id = generate_match_id(winner_id, loser_id, date, result, season)
                ...
                new_matches.append({
                    ...
                    'winner_id': winner_id,
                    'loser_id': loser_id,
                    'result': match.get('result', ''),  # Use raw result string
                    'event': match.get('event', ''),
                    'weight': normalize_weight(match.get('weight', 0)),
                    ...
                    'date': match.get('date', '')  # Add date field
                })
...
def is_name_variant_match(a, b):
    """
    Check if two names are exact matches or known variations.
    Returns True if:
    1. Names are exactly equal after normalization
    2. First names are known variations and rest of name matches exactly
    """
    ...
    if a_norm == b_norm:
        print(f"EXACT DEBUG: '{a}' and '{b}' are exact matches")
        return True
    ...
    if len(a_parts) > 0 and len(b_parts) > 0:
        ...
        # Check if first names are variants using the global NAME_VARIATIONS
        a_variants = NAME_VARIATIONS.get(a_first, {a_first})
        if b_first in a_variants:
            # Check if rest of name matches exactly
            a_rest = ' '.join(a_parts[1:])
            b_rest = ' '.join(b_parts[1:])
            if a_rest == b_rest:
                print(f"EXACT DEBUG: '{a}' and '{b}' match via name variations")
                return True
...
def class_to_num(cls):
    return {
        "FR": 1, "RSFR": 1, "Fr.": 1, "RS Fr.": 1,
        "SO": 2, "RSSO": 2, "So.": 2, "RS So.": 2,
        "JR": 3, "RSJR": 3, "Jr.": 3, "RS Jr.": 3,
        "SR": 4, "RSSR": 4, "Sr.": 4, "RS Sr.": 4
    }.get(cls, 0)
...
def evaluate_career_match(current, match):
    ...
    # Input features
    fr = int(current['class_year'] in {'FR', 'RSFR', 'Fr.', 'RS Fr.'})
    exact_name = is_name_variant_match(current['name'], match['name'])
    name_match = exact_name or is_fuzzy_match(current['name'], match['name'])
    team_match = int(current['team_id'] == match['team_id'])

    # Convert to integers for the lookup key
    fr_int = int(fr)
    exact_int = int(exact_name)
    name_match_int = int(name_match)
    team_match_int = int(team_match)

#    print(f"\n=== Running evaluate_career_match() ===")
#    print(f"Input values:")
#    print(f"  fr (freshman): {fr_int}")
#    print(f"  name_match (fuzzy/var): {name_match_int}")
#    print(f"  exact (exact/var): {exact_int}")
#    print(f"  team_match: {team_match_int}")

    try:
        weight_diff = abs(int(current['weight_class']) - int(match['weight_class']))
#        print(f"  weight_diff: {weight_diff}")
    except:
        weight_diff = 999
#        print(f"  weight_diff: Error calculating, using {weight_diff}")
    
    # Normalize weight score to 1 if non-zero
    raw_weight = 2 if weight_diff <= 10 else 1 if weight_diff <= 20 else 0
    weight = 1 if raw_weight > 0 else 0
#    print(f"  raw weight score: {raw_weight}")
#    print(f"  normalized weight score: {weight}")

    grade_diff = class_to_num(current['class_year']) - class_to_num(match['class_year'])
    grade_match = int(grade_diff in {0, 1})
#    print(f"  grade_diff: {grade_diff}")
#    print(f"  grade_match: {grade_match}")

    input_key = (fr_int, name_match_int, exact_int, team_match_int, weight, grade_match)
#    print(f"\nLooking up key in decision table: {input_key}")
    # Full 64-row lookup table (0/1 for each input bit)
    decision_table = {
        # Format: (fr, nm, ex, tm, wt, gm): (action, score)

        # Non-Freshman cases
        (0, 0, 0, 0, 0, 0): ('Add to Suspect List', 0),
        (0, 0, 0, 0, 0, 1): ('Add to Suspect List', 10),
        (0, 0, 0, 0, 1, 0): ('Add to Suspect List', 20),
        (0, 0, 0, 0, 1, 1): ('Add to Suspect List', 30),
        (0, 0, 0, 1, 0, 0): ('Add to Suspect List', 20),
        (0, 0, 0, 1, 0, 1): ('Add to Suspect List', 30),
        (0, 0, 0, 1, 1, 0): ('Add to Suspect List', 40),
        (0, 0, 0, 1, 1, 1): ('Add to Suspect List', 50),
        (0, 1, 0, 0, 0, 0): ('Add to Suspect List', 50),
        (0, 1, 0, 0, 0, 1): ('Add to Suspect List', 60),
        (0, 1, 0, 0, 1, 0): ('Add to Suspect List', 70),
        (0, 1, 0, 0, 1, 1): ('Add to Suspect List', 80),
        (0, 1, 0, 1, 0, 0): ('Add to Suspect List', 70),
        (0, 1, 0, 1, 0, 1): ('Add to Suspect List', 80),
        (0, 1, 0, 1, 1, 0): ('Add to Suspect List', 90),
        (0, 1, 0, 1, 1, 1): ('AUTO Link Career', 90),
        (0, 1, 1, 0, 0, 0): ('Add to Suspect List', 70),
        (0, 1, 1, 0, 0, 1): ('Add to Suspect List', 80),
        (0, 1, 1, 0, 1, 0): ('Add to Suspect List', 90),
        (0, 1, 1, 0, 1, 1): ('Add to Suspect List', 100),
        (0, 1, 1, 1, 0, 0): ('Add to Suspect List', 80),
        (0, 1, 1, 1, 0, 1): ('AUTO Link Career', 80),
        (0, 1, 1, 1, 1, 0): ('AUTO Link Career', 90),
        (0, 1, 1, 1, 1, 1): ('AUTO Link Career', 100),

        # Freshman cases
        (1, 0, 0, 0, 0, 0): ('AUTO New Career', 0),
        (1, 0, 0, 0, 0, 1): ('AUTO New Career', 10),
        (1, 0, 0, 0, 1, 0): ('AUTO New Career', 20),
        (1, 0, 0, 0, 1, 1): ('AUTO New Career', 30),
        (1, 0, 0, 1, 0, 0): ('AUTO New Career', 20),
        (1, 0, 0, 1, 0, 1): ('AUTO New Career', 30),
        (1, 0, 0, 1, 1, 0): ('AUTO New Career', 40),
        (1, 0, 0, 1, 1, 1): ('AUTO New Career', 50),
        (1, 1, 0, 0, 0, 0): ('AUTO New Career', 50),
        (1, 1, 0, 0, 0, 1): ('Add to Suspect List', 60),
        (1, 1, 0, 0, 1, 0): ('AUTO New Career', 70),
        (1, 1, 0, 0, 1, 1): ('Add to Suspect List', 80),
        (1, 1, 0, 1, 0, 0): ('Add to Suspect List', 70),
        (1, 1, 0, 1, 0, 1): ('AUTO Link Career', 80),
        (1, 1, 0, 1, 1, 0): ('Add to Suspect List', 90),
        (1, 1, 0, 1, 1, 1): ('AUTO Link Career', 100),
        (1, 1, 1, 0, 0, 0): ('AUTO New Career', 50),
        (1, 1, 1, 0, 0, 1): ('Add to Suspect List', 60),
        (1, 1, 1, 0, 1, 0): ('AUTO New Career', 70),
        (1, 1, 1, 0, 1, 1): ('Add to Suspect List', 80),
        (1, 1, 1, 1, 0, 0): ('Add to Suspect List', 70),
        (1, 1, 1, 1, 0, 1): ('AUTO Link Career', 80),
        (1, 1, 1, 1, 1, 0): ('Add to Suspect List', 90),
        (1, 1, 1, 1, 1, 1): ('AUTO Link Career', 100),
    }

    # Debug: check if key exists and what we're getting
    result = decision_table.get(input_key)
#    print(f"Result from table lookup: {result}")
    if result is None:
        print("Key not found in table! Using default.")
        # Debug: show some sample keys from the table
        print("\nFirst few keys in table:")
        for k in list(decision_table.keys())[:5]:
            print(f"  {k}")
        print("\nKey types in first table entry:")
        first_key = list(decision_table.keys())[0]
        print(f"  {first_key}: {[type(x).__name__ for x in first_key]}")
        print("\nOur input key types:")
        print(f"  {input_key}: {[type(x).__name__ for x in input_key]}")


    return decision_table.get(input_key, ('Add to Suspect List', 0))
...
def test_career_match_evaluator():
    ...
    def create_test_wrestlers(fr, nm, ex, tm, wm, gm):
        current_name = "John Smith"
        match_name = (
            "John Smith" if ex else
            "Johnny Smith" if nm else
            "Mike Jones"
        )
        current_team = "TEAM1"
        match_team = current_team if tm else "TEAM2"
        current_weight = 165
        match_weight = (
            current_weight if wm == 2 else
            current_weight - 15 if wm == 1 else
            current_weight - 30
        )
        current_class = "FR" if fr else "SO"
        match_class = (
            "FR" if current_class == "FR" and gm else
            "SR" if current_class == "FR" else
            "FR" if gm else "SO"
        )
        current = {
            'name': current_name,
