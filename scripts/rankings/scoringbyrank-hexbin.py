#!/usr/bin/env python3
"""
scoringbyrank-hexbin.py

Generate a match-level HEXBIN density plot showing the relationship between
rank and points scored for a given season (and optional date range).

Same inputs/outputs as the original scatter version:
  ...
  - CLI:
        python scoringbyrank-hexbin.py -season 2026 -startdate 120125 -enddate 123125
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Hexbin density plot of points scored vs rank for a season."
    )
...
def make_plot(
    ...
    """
    Hexbin density plot + LOWESS trend.
    """
    ...
    # HEXBIN density (log scale so dense areas pop)
    hb = plt.hexbin(
        ranks,
        points,
        gridsize=50,
    ...
    cb = plt.colorbar(hb)
    cb.set_label("Match density (log scale)")

    # LOWESS curve overlay (if available)
    if lowess is not None:
        try:
            smoothed = lowess(points, ranks, frac=0.2)
