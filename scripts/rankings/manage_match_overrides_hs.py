#!/usr/bin/env python3
"""
Manage manual match result overrides for High School.
...
File format (mt/rankings_data/hs_{state}_{gender}/{season}/match_overrides.json):
{
  "overrides": [
    {
      "wrestler1_id": "12345",
...
def load_overrides(season: int, state: str, gender: str, data_dir: str = "mt/rankings_data") -> Dict:
    """Load match overrides for a season (HS-specific path)."""
...
def save_overrides(season: int, state: str, gender: str, data: Dict, data_dir: str = "mt/rankings_data") -> None:
    """Save match overrides for a season (HS-specific path)."""
    state_lower = state.lower() if state else 'ky'
    overrides_path = Path(data_dir) / f"hs_{state_lower}_{gender}" / str(season) / "match_overrides.json"
...
def load_all_wrestlers(season: int, state: str, gender: str, data_dir: str = "mt/rankings_data") -> Dict[str, Dict]:
    """
    Load all wrestlers from weight_class_*.json files (HS-specific path).
...
def find_matching_matches(
    w1_id: str, w2_id: str, date: str, season: int, state: str, gender: str, data_dir: str = "mt/rankings_data"
) -> List[Dict]:
    """
    Find actual matches that would match an override (HS-specific path).
...
def list_overrides(season: int, state: str, gender: str, data_dir: str = "mt/rankings_data") -> None:
    ...
    print(f"\nMatch Overrides for Season {season} ({state} HS {gender.capitalize()}):")
    print("=" * 80)
...
def remove_override(season: int, state: str, gender: str, index: int, data_dir: str = "mt/rankings_data") -> None:
...
def interactive_add_override(season: int, state: str, gender: str, data_dir: str = "mt/rankings_data") -> None:
    ...
    add_override(
        season, state, gender, w1_id, w2_id, date, winner_id, result,
...
def main() -> None:
    ...
    if state_upper != 'KY':
        print(f"Warning: Only KY is currently fully supported. Proceeding with {args.state}...")
    ...
    if args.remove:
        remove_override(args.season, args.state, args.gender, args.remove, args.data_dir)
        return
