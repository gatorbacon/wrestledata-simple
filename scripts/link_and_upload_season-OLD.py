...
def evaluate_career_match(wrestler, match):
    """
    Evaluates a potential career match using the rules table approach.
    
    Args:
        wrestler: Current wrestler data dictionary
        match: Potential match wrestler data dictionary
        
    Returns:
        tuple: (action, confidence_score)
            action: 'AUTO Link Career', 'AUTO New Career', or 'Add to Suspect List'
            confidence_score: The calculated confidence score (0-100)
    """
    # Calculate individual components
    is_freshman = normalize_class_year(wrestler.get('class_year', '')) in ['FR', 'RSFR']
    
    # Name matching (50% of score)
    name_sim = name_similarity(wrestler.get('name', ''), match.get('name', ''))
    name_match = name_sim >= 0.4  # Any similarity above 40% counts as a match
    exact_name = is_exact_name_match(wrestler.get('name', ''), match.get('name', ''))
    name_score = 50 * name_sim  # Up to 50 points for name
    
    # Team matching (20% of score)
    team_match = wrestler.get('team_id') == match.get('team_id')
    team_score = 20 if team_match else 0
    
    # Weight matching (20% of score)
    w1 = normalize_weight(wrestler.get('weight_class'))
    w2 = normalize_weight(match.get('weight_class'))
    if w1 is not None and w2 is not None:
        weight_diff = abs(w1 - w2)
        weight_within_20lbs = weight_diff <= 20
        # For scoring: 20 points if within 10lbs, 10 points if within 20lbs
        weight_score = 20 if weight_diff <= 10 else (10 if weight_diff <= 20 else 0)
    else:
        weight_within_20lbs = False
        weight_score = 0
    
    # Grade progression matching (10% of score)
    current_grade = normalize_class_year(wrestler.get('class_year', ''))
    previous_grade = normalize_class_year(match.get('class_year', ''))
    grade_match = class_year_score(current_grade, previous_grade) > 0
    grade_score = 10 if grade_match else 0
    
    # Total confidence score (0-100)
    confidence_score = name_score + team_score + weight_score + grade_score
    
    # Debug output
    print(f"DEBUG: {wrestler.get('name')} vs {match.get('name')} - F:{is_freshman} NM:{name_match} EN:{exact_name} TM:{team_match} WT:{weight_within_20lbs} GM:{grade_match} - Score: {confidence_score}")
    
    # Check for Row 12 case specifically
    if is_freshman:
        # Directly check names to ensure they are truly different for Row 12
        name1 = wrestler.get('name', '').lower()
        name2 = match.get('name', '').lower()
        
        # If names are completely different or very low similarity
        if name1 != name2 and name_sim < 0.3:
            print(f"DEBUG: Names are different enough for Row 12: '{name1}' vs '{name2}', sim: {name_sim}")
            return 'AUTO New Career', confidence_score
    
    # Apply the rules table logic
    if is_freshman:
        # Now continue with the rest of the freshman rules (rows 13-28)
        if not exact_name:
            # Rows 13-20: Freshman with name match but not exact
            if not team_match:
                # Rows 13-16: Different team
                if not weight_within_20lbs and not grade_match:
                    return 'AUTO New Career', confidence_score  # Row 13
                elif not weight_within_20lbs and grade_match:
                    return 'Add to Suspect List', confidence_score  # Row 14
                elif weight_within_20lbs and not grade_match:
                    return 'AUTO New Career', confidence_score  # Row 15
                else:  # weight_within_20lbs and grade_match
                    return 'Add to Suspect List', confidence_score  # Row 16
            else:
                # Rows 17-20: Same team
                if not weight_within_20lbs and not grade_match:
                    return 'Add to Suspect List', confidence_score  # Row 17
                elif not weight_within_20lbs and grade_match:
                    return 'AUTO Link Career', confidence_score  # Row 18
                elif weight_within_20lbs and not grade_match:
                    return 'Add to Suspect List', confidence_score  # Row 19
                else:  # weight_within_20lbs and grade_match
                    return 'AUTO Link Career', confidence_score  # Row 20
        else:
            # Rows 21-28: Freshman with exact name match
            if not team_match:
                # Rows 21-24: Different team
                if not weight_within_20lbs and not grade_match:
                    return 'AUTO New Career', confidence_score  # Row 21
                elif not weight_within_20lbs and grade_match:
                    return 'Add to Suspect List', confidence_score  # Row 22
                elif weight_within_20lbs and not grade_match:
                    return 'AUTO New Career', confidence_score  # Row 23
                else:  # weight_within_20lbs and grade_match
                    return 'Add to Suspect List', confidence_score  # Row 24
            else:
                # Rows 25-28: Same team
                if not weight_within_20lbs and not grade_match:
                    return 'Add to Suspect List', confidence_score  # Row 25
                elif not weight_within_20lbs and grade_match:
                    return 'AUTO Link Career', confidence_score  # Row 26
                elif weight_within_20lbs and not grade_match:
                    return 'AUTO Link Career', confidence_score  # Row 27
                else:  # weight_within_20lbs and grade_match
                    return 'AUTO Link Career', confidence_score  # Row 28
    else:
        # Non-freshman rules (rows 1-11)
        if not name_match:
            # Row 1: Non-freshman without name match
            return 'Add to Suspect List', confidence_score
        elif not exact_name:
            # Rows 2-6: Non-freshman with name match but not exact
            if not team_match:
                # Rows 2-3: Different team
                return 'Add to Suspect List', confidence_score  # Rows 2 & 3
            else:
                # Rows 4-6: Same team
                if not weight_within_20lbs and not grade_match:
                    return 'Add to Suspect List', confidence_score  # Row 4
                elif not weight_within_20lbs and grade_match:
                    return 'Add to Suspect List', confidence_score  # Row 5
                else:  # Weight within 20lbs or grade match (Row 6)
                    return 'AUTO Link Career', confidence_score  # Row 6
        else:
            # Rows 7-11: Non-freshman with exact name match
            if not team_match:
                # Row 7: Different team
                return 'Add to Suspect List', confidence_score
            else:
                # Rows 8-11: Same team
                if not weight_within_20lbs and not grade_match:
                    return 'Add to Suspect List', confidence_score  # Row 8
                elif not weight_within_20lbs and grade_match:
                    return 'AUTO Link Career', confidence_score  # Row 9
                elif weight_within_20lbs and not grade_match:
                    return 'AUTO Link Career', confidence_score  # Row 10
                else:  # weight_within_20lbs and grade_match
                    return 'AUTO Link Career', confidence_score  # Row 11
...
def test_career_match_evaluator():
    ...
    test_cases = [
        ...
        # Row 12: Freshman, no name match
        {'row': 12, 'is_freshman': True, 'name_match': False, 'exact_name': False, 
         'team_match': False, 'weight_match': False, 'grade_match': False,
         'expected_action': 'AUTO New Career'},
