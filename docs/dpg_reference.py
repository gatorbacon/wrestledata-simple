#!/usr/bin/env python3
"""
DPG — Massey/BT Fusion : CANONICAL REFERENCE IMPLEMENTATION
============================================================
        ...
        for w in WEIGHTS:
            ...
            for i in pw["ids"]:
                rows.append(dict(
                    ...
                    dpg=round(fused_by_weight[w][i] - global_offset, 2),
                ))
        rows = [r for r in rows if r["matches"] >= MIN_MATCHES]
        rows.sort(key=lambda r: (-r["dpg"], r["name"]))   # deterministic
...
def main():
    ...
    for r in rows[:a.top]:
        print(f"  {r['rank']:3d}. {r['dpg']:+5.2f}  {r['name']} ({r['weight']}, {r['team']})  [{r['matches']}m]")
